...
//    List<PlanDetail> findByPlanID(@Param("planID") Integer planID);
    ...
    // Plan Detail
    @Query(value = "SELECT plan_detailid, plan_check, plan_day_number, plan_end_time, plan_location, plan_number, plan_price, plan_start_time FROM plan_detail pd WHERE pd.planID = :planID", nativeQuery = true)
    List<PlanDetail> findByPlanID(@Param("planID") Integer planID);
